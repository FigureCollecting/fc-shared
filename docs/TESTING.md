...
**Caveat — the compile-error guarantee holds ONLY when the consumer imports the producer's shared contract artifact.** Where a stack mirrors types locally instead of importing the shared source — fc-frontend's `src/types` (no `@figurecollecting/fc-shared` / proto / OpenAPI import) and image-manager's Pydantic models (FastAPI-generated OpenAPI + local Pydantic, no shared import) — a producer contract change does NOT become a compile error in the consumer; the local types silently diverge and drift is caught only at runtime/assertion. That is a **standing risk**, not a covered case. Mitigation: fc-frontend should consume `@figurecollecting/fc-shared` (or codegen its types from the backend OpenAPI); image-manager consumers should codegen from the producer's `/openapi.json`. Until then, treat these two boundaries (the interface between the engine and the data it loads) as assertion-only and weigh them accordingly.
...
Two non-negotiables:
1. **Use the real engine via Testcontainers**: `PostgresContainer` for image-manager; `apache/age:release_PG16_1.6.0` (`.asCompatibleSubstituteFor("postgres")`) for catalog-api. Run real migrations against it (see Migration testing).
...
> ❌ **image-manager** runs SQLite-as-Postgres (`@compiles(UUID/CITEXT, "sqlite")` + `is_postgres()` ILIKE fallback), so `to_tsvector`/`@@` full-text, `ARRAY`/`&&` tag-overlap, `CITEXT` case-insensitive uniqueness, and native `UUID` get **zero** test coverage while the suite stays green.
> ❌ **catalog-api** runs `postgres:16-alpine`, which has no Apache AGE, so `cypher()`/`CypherQueryBuilder` and `V2__enable_age_extension.sql` are silently skipped.

→ Per stack: see Part II (image-manager and catalog-api playbooks carry the concrete container + smoke-test wiring).
...
- **Bring the ungated laggards up to at least the floor:** `fc-mobile`, `catalog-android`, `image-manager`, `fc-lookup`, and `scraper-rulesets` currently measure-but-don't-enforce; each must add a gate at ≥70/70 and only move up from there.
...
- **`catalog-api` keeps its enforced 85** (JaCoCo `jacocoTestCoverageVerification` wired into `check`) — it is the best example to copy, not the exception.
...
**Be honest: §8's automated checks are NOT yet built.** Today these rules describe where we are going, not where we are. As of this writing: **MSW is in ZERO repos**; the **§8 grep guard exists in NO workflow**; **`renderAuthenticated` does not exist** and `fc-frontend/src/test-utils.tsx` **still does not seed auth**; coverage gates are **partial** (jest `coverageThreshold` lives only in fc-frontend, at a low 40-60; absent in backend/scraper/fc-shared; fc-lookup has no test script at all); and **catalog-android's MockWebServer is declared-but-unused**. The **only live automated control** over the qualitative failure classes is **Codecov coverage %** — which these rules themselves (§5, §10 living-lessons) say catch **none** of these classes (test-that-can-never-fail, client-mock, real-time-delay, reachability, DB-engine all produce coverage). So shipping the rules without the rollout would just repeat §0's history.
...
- Add **MSW v2** to the four TS-Jest repos **+ fc-frontend**; **respx** to image-manager; switch **catalog-android** to its already-declared **MockWebServer**.
...
**fc-shared is the first adopter.** Its API trio — `client` / `figures` / `scraper` — with MSW is **Phase-1, step 1**, and the **reference implementation** every other repo copies. fc-shared owns the network code that fc-mobile's typed-module mock relies on (§4 exception), so getting its MSW contract suite right is what makes that whole pattern honest downstream.
...
**Fake the network connection (HTTP/external):** WireMock at the process connection — stub the HTTP server, never mock the `RestClient`. catalog-api already does this correctly:
```java
@WireMockTest
...
### Playbook: Kotlin + JUnit4 + Android / Jetpack Compose (catalog-android)

**Current state:** catalog-android is an offline-first Compose app (AGP 9.0.1, Kotlin 2.2.10, Hilt, Room, Retrofit/Moshi) with a strong JVM unit suite already: MockK, Turbine, `kotlinx-coroutines-test`, Robolectric-driven Compose `ui-test`, and a Room in-memory androidTest. Two gaps must be closed by this playbook: there is **no CI** (`.github/workflows` is absent, unlike the `catalog-api` sibling which ships `build.yml`/`codeql.yml`/`security-scan.yml`), and coverage is **report-only** — `tasks.register<JacocoReport>("jacocoTestReport")` emits XML/HTML but enforces no threshold. A latent gap: `okhttp-mockwebserver` is declared but used by zero tests — every network test mocks the Retrofit `*Api` interface directly (`mockk<AuthApi>()`, hand-rolled `FakeCollectionApi`), so the Moshi+Retrofit serialization boundary is never crossed.
